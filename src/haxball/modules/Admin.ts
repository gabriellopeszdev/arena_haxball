...
export class AdminModule {
  ...
  public adm(execInfo: CommandExecInfo): void {
    ...
    if (!this.hasAdmin() || ["👮‍♂️ capitão", "💂 sub-capitão", "⚽ jogador"].includes(player.settings.role)) {
      player.admin = true;
  ...
  onPlayerAdminChange(changedPlayer: Player, byPlayer?: Player): void {
    ...
    const hierarchy: Record<string, number> = { "👮‍♂️ capitão": 4, "💂 sub-capitão": 3, "⚽ jogador": 2, admin: 1 };
