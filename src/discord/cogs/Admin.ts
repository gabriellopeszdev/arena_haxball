...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (["👮‍♂️ capitão", "💂 sub-capitão", "⚽ jogador"].includes(player.settings.role)) { await interaction.reply({ embeds: [EmbedFactory.createErrorEmbed(`❌ ${player.name} é ${player.settings.role}.`, user)], flags: MessageFlags.Ephemeral }); return; }
  player.admin = status;
